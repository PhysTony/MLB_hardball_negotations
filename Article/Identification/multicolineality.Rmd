...
run_individual_linear_regressions <- function(df, column_names, target_variable, exclude_variables = NULL) {
  ...
  # Lista para almacenar los modelos y sus R^2
  models <- list()
  r_squared <- list()
  ...
  for (column in column_names) {
    ...
    # Agregar el modelo y su R^2 a las listas
    models[[column]] <- model
    r_squared[[column]] <- summary(model)$r.squared
  }
  ...
  for (i in seq(1, length(models), by = 5)) {
    ...
    subset_r_squared <- r_squared[i:min(i+4, length(models))]
    ...
    # Imprimir los valores de R^2
    cat("Valores de R^2 para el Grupo", ceiling(i / 5), ":\n")
    for (name in names(subset_r_squared)) {
      cat(name, ": R^2 =", subset_r_squared[[name]], "\n")
    }
  }
}
```
