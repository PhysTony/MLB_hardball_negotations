...
hitters_fd_panel <- read.csv('ETL_Data/Panel/Cumulative/Dynamic_model/result_hitter.csv')
fielders_fd_panel <- read.csv('ETL_Data/Panel/Cumulative/Dynamic_model/result_fielder.csv')
```
...
# Calcular el cuadrado de Edad_t y Anios_de_contrato_t
hitters_panel$Edad_t_2 <- hitters_panel$Edad_t^2
hitters_panel$Anios_de_contrato_t_2 <- hitters_panel$Anios_de_contrato_t^2
fielders_panel$Edad_t_2 <- fielders_panel$Edad_t^2
fielders_panel$Anios_de_contrato_t_2 <- fielders_panel$Anios_de_contrato_t^2
...
# Control variables:
vars_ms  <- 'Y ~ Edad_t_2 + Anios_de_contrato_t_2 + I(team_num_t^2) + X_Ingresos_equipo_t + X_Ingresos_equipo_t_1'
vars_fe  <- 'Y ~ Edad_t_2 + Anios_de_contrato_t_2 + I(team_num_t^2) + X_Ingresos_equipo_t + X_Ingresos_equipo_t_1 - 1'
...
```{r multicolieanility_cojoint_fielder, echo=FALSE, eval=TRUE}
convert_to_lm <- function(plm_model, data) {
    formula <- formula(plm_model)
    lm_model <- lm(formula, data)
    return(lm_model)
}

# Convertir los primeros tres modelos usando starting_data
lm_models <- lapply(fielder_end_models[1:3], convert_to_lm, data = starting_data)

# Convertir el cuarto modelo usando fielders_fd_panel
lm_models$fd <- convert_to_lm(fielder_end_models$fd, data = fielders_fd_panel)

vif_results <- lapply(lm_models, vif)
```
...
```{r simple_identification_hitters, echo=FALSE}
# Lista de bases de datos
data_list <- list(hitters_panel, hitters_panel, hitters_panel, hitters_fd_panel)
