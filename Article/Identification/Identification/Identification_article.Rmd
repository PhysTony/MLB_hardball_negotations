...
hitter_data_list_2 <- list(hitters_panel, hitters_panel, hitters_fd_panel)
fielder_data_list_2 <- list(fielders_panel, fielders_panel, fielders_fd_panel)

# Lista de modelos a aplicar
model_list <- list("pooling", "within", "random", "random")
model_list_2 <- list("pooling", "random", "random")

# Lista de nombres para imprimri
name_list <- list("Pooling", "Within", "RE", "FD")
name_list_2 <- list("Pooling", "RE", "FD")
...
Quitaremos la variable del periodo $t$ y el estimador *within*:
...
stargazer(hitter_simple_stimation_2,
         ...
         type = "text",
         title = "Bateadores: Comparación de los modelos",
         column.labels = c("Pooling", "Random effects","First-Differences"),
...
En este caso, la variable innecesaria corresponde a la variable rezagada, así como la *within*.
...
# Fórmula para el modelo
formula_general_1 <- 'Y ~ Edad_t_2 + Anios_de_contrato_t_2 + I(team_num_t^2) + X_Ingresos_equipo_t_1 - 1'

# Iterar sobre cada base de datos y cada modelo
for (i in 1:3) {
  # Estimar el modelo usando plm
  hitter_general_stimation_1[[length(hitter_general_stimation_1) + 1]] <- plm(formula_general_1,
                                                                            data = hitter_data_list_2[[i]],
                                                                            model = model_list_2[[i]],
                                                                            index = c("id", "Anio_ref"))
...
Quitaremos todos los controles a excepción de la edad cuadrada en el caso del estimador *pooling*

```{r general_identification_hitters_2, echo=FALSE, warning=FALSE}
# Inicializar una lista para almacenar los modelos
hitter_general_stimation_2 <- list()

# Inicializar una lista para almacenar los resultados
hitter_general_stimation_results_2 <- list()

# Iterar sobre cada base de datos y cada modelo
for (i in 1:3) {
  # Fórmula para el modelo
  if(i==1 | i==3){
    formula_general_2 <- 'Y ~ Edad_t_2 + X_Ingresos_equipo_t_1 - 1'
  }
  else{
    formula_general_2 <- formula_simple_2
  }
  # Estimar el modelo usando plm
  hitter_general_stimation_2[[length(hitter_general_stimation_2) + 1]] <- plm(formula_general_2,
                                                                            data = hitter_data_list_2[[i]],
                                                                            model = model_list_2[[i]],
                                                                            index = c("id", "Anio_ref"))
  
  # To store the results
  hitter_general_stimation_results_2[[length(hitter_general_stimation_results_2) + 1]] <- coeftest(hitter_general_stimation_2[[length(hitter_general_stimation_2)]],
      vcov = vcovHC(hitter_general_stimation_2[[length(hitter_general_stimation_2)]],
                                                type = "HC1",
                                                cluster = "group"))
}

# Print the third block of results
stargazer(hitter_general_stimation_2,
         no.space = TRUE,
         align = TRUE,
         type = "text",
         title = "Bateadores: Comparación de los modelos",
         column.labels = c("Pooling", "Random effects","First-Differences"),
         covariate.labels = c("Edad$^{2}_{t}$",
                              "X$_{tearev}_{t-1}$"))
...
```{r general_identification_fielders_2, echo=FALSE, warning=FALSE}
# Inicializar una lista para almacenar los modelos
fielder_general_stimation_1 <- list()

# Inicializar una lista para almacenar los resultados
fielder_general_stimation_results_1 <- list()

# Fórmula para el modelo
formula_general_1 <- 'Y ~ Edad_t_2 + Anios_de_contrato_t_2 + I(team_num_t^2) + X_Ingresos_equipo_t - 1'

# Iterar sobre cada base de datos y cada modelo
for (i in 1:3) {
  # Estimar el modelo usando plm
  fielder_general_stimation_1[[length(fielder_general_stimation_1) + 1]] <- plm(formula_general_1,
                                                                            data = fielder_data_list_2[[i]],
                                                                            model = model_list_2[[i]],
                                                                            index = c("id", "Anio_ref"))
  
  # To store the results
  fielder_general_stimation_results_1[[length(fielder_general_stimation_results_1) + 1]] <- coeftest(fielder_general_stimation_1[[length(fielder_general_stimation_1)]],
      vcov = vcovHC(fielder_general_stimation_1[[length(fielder_general_stimation_1)]],
                                                type = "HC1",
                                                cluster = "group"))
}

# Print the third block of results
stargazer(fielder_general_stimation_1,
         no.space = TRUE,
         align = TRUE,
         type = "text",
         title = "Lanzadores: Comparación de los modelos",
...
Ningún control es estadísticamente significativo por lo queda el último modelo obtenido para los lanzadores iniciales en la identificación simple.


# Identificación con Medidas de Desempeño

Emplearemos las medidas de desempeño, según la posición ocupada, de los jugadores como controles
