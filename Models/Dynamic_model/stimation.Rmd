...
```{r}
# Assuming 'panel_data' is your panel data frame with the following columns:
# 'Jugador' (player name), 'Salario' (salary), and 'Year' (year of observation)
...
plm_data <- pdata.frame(hitter_data_did,
                        index = c("Jugador","Anio_did"))

# Estimate DID model with multiple periods
did_model <- plm(Y_Sueldo_regular_norm_t ~ treatment * factor(Anio_did > 0),
                 data = plm_data,
                 model="within")

# Extract ATE estimate from DID model
ate_estimate <- coef(did_model)["treatment:factor(Anio_did >= 0)TRUE"]

# Plot ATE estimate
ggplot(data=data.frame(ate_estimate), aes(x = "", y = ate_estimate)) +
