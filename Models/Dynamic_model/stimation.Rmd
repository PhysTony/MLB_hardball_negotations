...
cum_hitter <- lapply(cum_hitter, function(x) paste(x, "_cum", sep = ""))
ya_hitter <- lapply(ya_hitter, function(x) paste(x, "_ya", sep = ""))

# Lista con todas las variables
hitter_var <- c(cum_hitter, ya_hitter)
```
...
for (i in 1:length(hitter_var)){
  ...
  model <- lm(formula, data = hitter_data)
  model_robust <- coeftest(model, vcov = vcovHC(model, type = "HC1"))
  
  # store the results in the list
  hitter_results[[i]] <- list(variable = names(hitter_var[i]),
                              model = model_robust)
