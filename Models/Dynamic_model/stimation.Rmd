...
formula <- as.formula("Y_Sueldo_regular_norm_t ~ treatment * factor(Anio_did >= 0)")

# Estimate DID model with multiple periods
did_model <- plm(formula,
                 data = plm_data,
                 model = "within")


# Extract ATE estimate from DID model
print(ate_estimate <- coef(did_model)[3])
```
...
# Create plot
ggplot(data = parallel_means, aes(x = Anio_did, y = Y_Sueldo_regular_norm_t, color = Tratamiento)) +
  geom_line() +
