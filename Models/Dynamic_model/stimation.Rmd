...
stargazer(hitter_models,
         ...
         covariate.labels = c("$Edad_{t}$" , "Años contrato$_{t}$", "Eqipo$_{t}$", 
                              ...
                              "$X_{GS_{t}}$", "$X_{OPS_{t}}$", "$X_{OPS^{2}_{t}}$",
                              ...
                              "$X_{GS_{t-1}}$", "$X_{OPS_{t-1}}$", "$X_{OPS^{2}_{t-1}}$",
...
stargazer(hitter_models,
         ...
         align = TRUE,
         type = "text",
         title = "Bateadores: Comparación de los modelos - Tercer refinamiento",
         column.labels = c("Pooling", "Within", 
                           "Random effects","First-Differences"))
```
Ahora se refinará con respecto al signo puesto que no se espera ningún signo negativo en estas variables

```{r hitter_stimation_5}
# Significant variables:
# Pooling:
hitter_vars_1 <- c("X_Triples_t_1",
                    "X_WAR_t")
...
stargazer(hitter_models,
         ...
         align = TRUE,
         type = "text",
         title = "Bateadores: Comparación de los modelos - Cuarto refinamiento",
         column.labels = c("Pooling", "Within", 
                           "Random effects","First-Differences"))
```
```{r hitter_stimation_6}
# Significant variables:
# Pooling:
hitter_vars_1 <- c("X_Triples_t_1",
                    "X_WAR_t")
...
# First Differences
hitter_vars_4 <- c("X_Bateos_promedio_t_1",
                   "X_Juegos_iniciados_t_1",
                   "X_Porcentaje_on_base_2_t",
                   "X_WAR_t","X_WAR_2_t")
# Lista 
...
# Models
hitter_models_end <- list(pooling = hitter_results_stimation_1,
                          within = hitter_results_stimation_2,
                          random = hitter_results_stimation_3,
                          fd = hitter_results_stimation_4)
...
# Print the third block of results
stargazer(hitter_models_end,
         no.space = TRUE,
         align = TRUE,
         type = "text",
         title = "Bateadores: Comparación de los modelos - Refinamiento final",
         column.labels = c("Pooling", "Within", 
                           "Random effects","First-Differences"))
```
...
stargazer(fielder_models,
         ...
         column.labels = c("Pooling", "Within", 
                           "Random effects","First-Differences"))
```
Ahora omitamos las variables cuyo estimador tiene un signo que no tiene sentido. Veamos sí podemos corregir el signo del EHIP en el siguiente refinamiento, así como el de las BB en contra

```{r fielder_stimation_pooling_4}
# Significant variables:
...
stargazer(fielder_models,
         ...
         align = TRUE,
         type = "text",
         title = "Lanzadores Iniciales: Comparación de los modelos - Tercer refinamiento",
         column.labels = c("Pooling", "Within", 
                           "Random effects","First-Differences"))
```

```{r fielder_stimation_pooling_5}
# Significant variables:
fielder_vars_1 <- c('X_Control_t',
                    'X_Dominio_t_1',
                    'X_ERA_t_1',
...
stargazer(fielder_models,
         ...
         align = TRUE,
         type = "text",
         title = "Lanzadores Iniciales: Comparación de los modelos - Cuarto refinamiento",
         column.labels = c("Pooling", "Within", 
                           "Random effects","First-Differences"))
```

```{r fielder_stimation_pooling_6}
# Significant variables:
fielder_vars_1 <- c('X_Dominio_t_1',
                    'X_ERA_t_1',
                    'X_ERA_t',
                    'X_Saves_t_1',
                    '-1')
...
# Modelos
fielder_models_end <- list(pooling = fielder_results_stimation_1,
                           within = fielder_results_stimation_2,
                           random = fielder_results_stimation_3,
                           fd = fielder_results_stimation_4)
...
Tanto para bateadores comunes y bateadores iniciales, se filtraron las variables para obtener el modelo conjunto más adecuado.
...
for (j in 1:hitter_rep_ch){
  ...
  for (i in 1:hitter_stat_num){
    # Run linear regression with grouped errors by country and robust errors
    base_vars_h  <- paste(vars, stat_hitter_t_ch[[i + hitter_stat_num*(j - 1)]],
                        sep = '+')
    formula <- paste(base_vars_h,
  ...
  if (j == 3){
    for (i in 1:4){
    # Run linear regression with grouped errors by country and robust errors
    base_vars_h  <- paste(vars, stat_hitter_t_ch[[i + hitter_stat_num*(j)]],
                        sep = '+')
    formula <- paste(base_vars_h,
...
for (j in 1:fielder_rep_ch){
  ...
  for (i in 1:fielder_stat_num){
    # Run linear regression with grouped errors by country and robust errors
    base_vars_h  <- paste(vars, stat_fielder_t_ch[[i + fielder_stat_num*(j - 1)]],
                        sep = '+')
    formula <- paste(base_vars_h,
  ...
  if (j == 4){
    for (i in 1:2){
    # Run linear regression with grouped errors by country and robust errors
    base_vars_h  <- paste(vars, stat_fielder_t_ch[[i + fielder_stat_num*(j)]],
                        sep = '+')
    formula <- paste(base_vars_h,
...
formula <- as.formula("Y_Sueldo_regular_norm_t ~ treatment * factor(Anio_did >= 0) + Anios_de_contrato_t")
...
print(hitter_ate_estimate <- coef(hitter_did_model)[4])
```
...
# Extract ATE estimate from DID model
print(starting_ate_estimate <- coef(starting_did_model)[4])
```
...
ggplot(data = parallel_means,
  ...
  labs(color = "Agente libre")
# Save the plot as a PDF file
ggsave("did_model_plot_starting_w.pdf", width = 10, height = 8, dpi = 300)
```