...
hitter_results_simple_pooling_5 <- list()
hitter_results_simple_pooling <- list(result_1 = hitter_results_simple_pooling_1,
                                      ...
                                      result_5 = hitter_results_simple_pooling_5)
...
for (j in 1:4){
  ...
  for (i in 1:6){
    ...
    base_vars_h  <- paste(vars, stat_hitter_t[[i + 6*(j - 1)]],
                        sep = '+')
    formula <- paste(base_vars_h,
                     stat_hitter_t_1[[i + 6*(j - 1)]],
                     sep = " + ")
    
    hitter_simple_pooling[[i + 6*(j - 1)]] <- plm(formula, data = hitter_data,
                                                  model = "pooling",
                                                  index = c("id", "Anio_ref"))
        
    hitter_results_simple_pooling[[j]][[i]] <- coeftest(hitter_simple_pooling[[i + 6*(j - 1)]],
                                                        vcov = vcovHC(hitter_simple_pooling[[i + 6*(j - 1)]],
                                                                      type = "HC1",
                                                                      cluster = "group"))
      }
